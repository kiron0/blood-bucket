...
export default function ImageChangeModal() {
  ...
  const [changeProfilePicture, { data, isSuccess, error, isLoading }] =
    useState([]);
  const handleChangeProfile = handleSubmit(async (dataImage) => {
    const imageInfo = dataImage?.profileImage[0];
    const formData = new FormData();
    formData.append("photoURL", imageInfo);
  ...
  return (
    <form encType="multipart/form-data" onSubmit={handleChangeProfile}>
      <input
        type="checkbox"
